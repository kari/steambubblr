...
In this analysis I'll use a data set ranging between `r format(min(steam_stats$date), format = "%B %Y")` and `r format(max(steam_stats$date), format = "%B %Y")`, with (almost) daily data from mid-August 2016 onwards (except around May 2018). The data collection method was a bit spotty in the beginning.
...
(steam_stats |>
  ...
  filter(count > 1) |> arrange(-count) |> head(25))
...
With some annual franchises like <cite>Football Manager</cite> and 2K's NBA - the player base quickly migrates to the next installation a(nd sometimes like with the first franchise, the old game is removed from the Steam storefront). These installments could be grouped to one game for our purposes, but where should we set the limit? Should we do similar treatment for <cite>Call of Duty</cite> or just for annual sports franchises?
...
We can try to quickly group these games a bit by clustering. The attributes used in clustering are the slope, days in top list and the r^2. We'll also this time include all the games, not just those that have a positive trend to also identify games that hvae more product-like activity curves.
